<script lang="ts">
    ...
    The toolbar and the marker popups are both implemented by embedding Svelte components inside Leaflet elements. The marker and lines are toggled by updating the leaflet from reactive statements.
    ...
    */
    ...
    import * as markerIcons from '$lib/markers';
    let map;
    ...
    // of modifying the leaflet with reactive syntax.
    $: if(markerLayers && map) {
        if(eye) {
            markerLayers.addTo(map);
        } else {
