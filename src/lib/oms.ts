...
export default class OverlappingMarkerSpiderfier {
	...
	public trigger(event, ...args) {
		return Array.from(this.listeners[event] != null ? this.listeners[event] : []).map((func: CallableFunction) =>
	...
	public generatePtsCircle(count, centerPt) {
		...
		return (() => {
			...
			for (let i = 0, end = count, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
				...
				result.push(
					new this.leaflet.Point(centerPt.x + legLength * Math.cos(angle), centerPt.y + legLength * Math.sin(angle))
				);
			}
			return result;
		})();
	...
	public generatePtsSpiral(count, centerPt) {
		...
		return (() => {
			...
			for (let i = 0, end = count, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
				...
				const pt = new this.leaflet.Point(
